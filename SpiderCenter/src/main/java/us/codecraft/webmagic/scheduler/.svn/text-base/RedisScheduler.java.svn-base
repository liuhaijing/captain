...
//	@Override
	public boolean isDuplicate(Request request, Task task) {
		...
		Jedis jedis = pool.getResource();
		try {
			// 将直接缓存url改为缓存url的32位md5码
			String md5hex = DigestUtils.md5Hex(request.getUrl());
			boolean isDuplicate = jedis.sismember(getSetKey(task), md5hex);
			if (!isDuplicate) {
				jedis.sadd(getSetKey(task), md5hex);
			}
			return isDuplicate;
		} finally {
			pool.returnResource(jedis);
