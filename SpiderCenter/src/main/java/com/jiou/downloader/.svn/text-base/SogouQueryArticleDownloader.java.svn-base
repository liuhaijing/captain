...
public class SogouQueryArticleDownloader extends HttpClientDownloader {
	...
	public SogouQueryArticleDownloader() {
		super();
	}
		...
		try {
			...
			if (page != null) {
				if (checkLimit(page.getRawText())) {
					int temp =0;
					while (checkLimit(page.getRawText())&&temp<3) {
						 temp++;
						 cookieManager.put(cookieStr, false);
						 cookieStr = cookieManager.get();
						 try {
							page = httpDownload(request, task, cookieStr, page);
						} catch (Exception e) {
