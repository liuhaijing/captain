...
public class SogouFreqUrlPipeline implements Pipeline {
	...
	protected BloomFilter<String> bloomFilter = new FilterBuilder(5000000, 0.0001)
			.name(SogouFreqUrlPageProcessor.filterName)//
	...
	protected void saveArticle(ResultItems resultItems) {
		try {
			...
			doc.put(query_data_source, resultItems.get(query_data_source));
			artColl.insert(doc);
			bloomFilter.add(uid);
		} catch (Exception ex) {
			logger.error("url 插入文章内容错误", ex);
		}
	}
	...
	protected void saveNums(ResultItems resultItems) {
		try {
			...
			logger.error("url 插入文章阅读数错误", ignore);
		}
	}
}
